...
def movement_phase(game_state):
    # Obține ID-ul teritoriului curent
    current_territory_id = game_state['current_territory_id']
    
    # Obține forțele din teritoriul curent
    current_territory_forces = game_state['teritory'][current_territory_id]['forces']
    
    # Obține teritoriile vecine
    neighboring_territories = game_state['teritory'][current_territory_id]['neighboring_territories']
    
    # Sortează teritoriile vecine în funcție de numărul de forțe
    sorted_territories = sorted(neighboring_territories, key=lambda x: game_state['teritory'][x]['forces'], reverse=True)
    
    # Găsește primul teritoriu vecin cu mai puține forțe decât teritoriul curent
    target_territory = None
    for territory in sorted_territories:
        if game_state['teritory'][territory]['forces'] < current_territory_forces:
            target_territory = territory
            break
    
    # Dacă nu se găsește niciun teritoriu țintă, trece rândul
    if target_territory is None:
        return {'action': 'pass'}
    
    # Mută forțele din teritoriul curent în teritoriul țintă
    forces_to_move = min(current_territory_forces - game_state['teritory'][target_territory]['forces'], current_territory_forces)
    return {'action': 'move', 'from': current_territory_id, 'to': target_territory, 'forces': forces_to_move}
