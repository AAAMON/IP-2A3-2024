...
def bidding(game_state):
    ...
    bidding_aggressively = 1
    for card in my_cards:
        if card == "Lasgun":
            bidding_aggressively = 0
...
def revival(game_state):
    #TODO gene gesserit are low revival rate, dar are 1 free revival
    my_spice = game_state['Faction_Knowledge'][0]['Spice']
    nr_dead = game_state['Tleilaxu_Tanks'][0]['Forces'][faction_name]
    nr_revive = 0
    if nr_dead >0:
        nr_revive = 1
    return {"action": "revive", "value": nr_revive}
...
def movement(game_state): #TODO check if correct with json
    best_score = 0
    base_territory_id = 0
    best_territory_id = 0
    best_section_id = -1
