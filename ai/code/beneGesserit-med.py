...
def aliance(game_state):
    ...
    strong_aliances=['Atreides', 'Harkonen','Emperor']
...
def bidding(game_state):
    ...
    #bene gesserit's first priority is to grab as many weapons and defenses as possible. then use the voice in battle
    coef = 0.3 #instead of 0.2
...
def bene_shipment(game_state):
    #TODO de unde stiu pe ce teritoriu ma duc????
    my_reserves = game_state['Reserves'][0][faction_name]
    if my_reserves > 0:
        return{
            'action': 'bene_shipment',
            'answer': 'y'
            } 
    else:
        return{
...
def voice_use(game_state, defense, attack):
    if game_state['Round'] < 6:
    #in early Game focus on survival, protect generals to sustain forces
        if defense in projectile_defense:
            return 'poison' #cannot use poison
        if defense in poison_defense:
            return 'projectile'
    
    #in late game Focus on decisive victories: killing the opponent's general
    if attack in projectile_weapons:
        return 'shield'  #cannot use shield
    if attack in poison_weapons:
        return 'snooper'
    
    return None

def get_general_power(general):
    for faction, generals in generals_power.items():
        if general in generals:
            return generals[general]
    return None
                ...
                for attack in my_available_attack:
                    ...
                    for other_general in generals[enemy]:
                        for i in range(0,10):
                            ...
                            #TODO not implemented by api
                            #voice = voice_use(game_state, defense, attack)
                            #
                            #if voice == 'poison' and other_attack in poison_weapons:
                            #    other_attack = None  
                            #if voice == 'projectile' and other_attack in projectile_weapons:
                            #    other_attack = None
                            #if voice == 'shield' and other_defense in projectile_defense:
                            #    other_defense = None
                            #if voice == 'snooper' and other_defense in poison_defense:
                            #    other_defense = None
                            ...
                            my_power = forces
                            if my_general_alive:
                                my_power += get_general_power(general)
                                
                            other_power = other_forces
                            if other_general_alive:
                                other_power += get_general_power(other_general)
