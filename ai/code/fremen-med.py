...
faction_name = "Fremen"
nr_free_revive = 3
other_factions = ['Bene_Gesserit', 'Atreides', 'Harkonnen', 'Spacing_Guild', 'Emperor']
...
def evaluate_territory(game_state, territory):
    ...
    if territory['Name'] in stronghold_territories and cnt_oponents == 1 and simulate_battle(game_state, territory, my_forces + my_reserves // 2)[0] > 0.5:
...
##ok
    ...
    if phase_name == "Set-up" and phase_moment == 'initial faction forces placement':
        return {'action': 'forces set up', 'section': 67, 'value': 10}
##ok    
    if phase_name == "Storm":
