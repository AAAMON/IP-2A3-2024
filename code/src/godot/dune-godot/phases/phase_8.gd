...
func _ready():
	...
	var error = phase8InfoRequest.request(PlayerData.api_url + "get_phase_8_info")
	if error != OK:
		push_error("ERROR: HTTP: GET_PHASE_8_INFO")
...
func _on_phase_8_info_request_completed(_result, _response_code, _headers, body):
	...
	if (json == null):
		...
		if (firstCheck):
			var phase8Message = get_node("MessageBox/Description")
			var meToo = false;
			for i in range(json["howMany"]):
				...
				if (playerId == PlayerData.turnId):
					...
					phase8Message.text = "You received " + str(addedSpice) + " spice from " + json["whichPlayers"][i]["fromTerritory"]
					PlayerData.spice += addedSpice
					meToo = true;
				if (meToo == false):
					phase8Message.text = "You didn't receive spice as charity..."
				# update other
			firstCheck = false
